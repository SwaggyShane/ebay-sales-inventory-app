    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-gray-800 rounded-lg shadow p-4">
          <div className="text-gray-400 text-xs font-semibold uppercase">Lifetime Spent</div>
          <div className="text-2xl font-bold text-gray-100 mt-1">
            ${parseFloat(customer.lifetime_spent || 0).toFixed(2)}
          </div>
        </div>
        <div className="bg-gray-800 rounded-lg shadow p-4">
          <div className="text-gray-400 text-xs font-semibold uppercase">Purchases</div>
          <div className="text-2xl font-bold text-gray-100 mt-1">{customer.purchase_count}</div>
        </div>
        <div className="bg-gray-800 rounded-lg shadow p-4">
          <div className="text-gray-400 text-xs font-semibold uppercase">Avg Ticket</div>
          <div className="text-2xl font-bold text-gray-100 mt-1">
            ${parseFloat(customer.average_ticket || 0).toFixed(2)}
          </div>
        </div>
        <div className="bg-gray-800 rounded-lg shadow p-4">
          <div className="text-gray-400 text-xs font-semibold uppercase">Status</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-gray-800 rounded-lg shadow">
            <div className="border-b px-6 py-4">
              <h2 className="text-lg font-bold text-gray-100">Sales History</h2>
            </div>
                  ...
                  {sales.length === 0 ? (
                    ...
                    sales.map((sale) => (
                      <tr key={sale.id} className="border-b hover:bg-gray-700">
                        ...
                        <td className="px-6 py-3 text-sm text-gray-100">{sale.item_title}</td>
        ...
        <div className="lg:col-span-1">
          <div className="bg-gray-800 rounded-lg shadow">
            <div className="border-b px-6 py-4">
              <h2 className="text-lg font-bold text-gray-100">Add Note</h2>
            </div>
            <form onSubmit={handleAddNote} className="p-6 space-y-4">
              <div>
                <label htmlFor="noteType" className="block text-sm font-medium text-gray-300 mb-1">
                  Type
                </label>
                <select
                  id="noteType"
                  ...
                  className="w-full px-3 py-2 bg-gray-700 text-gray-100 border border-gray-600 rounded-md focus:ring-2 focus:ring-blue-400 outline-none text-sm"
                >
              ...
              <button
                type="submit"
                className="w-full py-2 bg-blue-500 hover:shadow-lg hover:shadow-blue-500/50 text-white font-semibold rounded-md text-sm"
              >
                Add Note
              </button>
            </form>
          ...
          <div className="bg-gray-800 rounded-lg shadow mt-6">
            <div className="border-b px-6 py-4">
              <h2 className="text-lg font-bold text-gray-100">Notes</h2>
            </div>
            <div className="divide-y max-h-96 overflow-y-auto">
              {notes.length === 0 ? (
                <div className="px-6 py-4 text-center text-gray-400">No notes</div>
              ) : (
                notes.map((note) => (
                  <div key={note.id} className="p-4 text-sm">
                    <div className="flex justify-between items-start mb-1">
                      <span className="font-semibold text-gray-100">{note.note_type}</span>
                      {note.is_resolved && (
